...
**Infraestructura self-hosted basada en tres nodos, manejada por contenedores en pods de Podman y servicios de systemd a nivel usuario**
...
> [!Note]
> [Documentación web](https://homelab.ncorrea.com.ar)
...
Cada dispositivo tiene su propia carpeta en este repo, con sus propios pods y sus propias variables de entorno.

## Stack

| Capa                  | Tecnología                                            |
| --------------------- | ----------------------------------------------------- |
| Contenerización       | Podman Compose, servicios systemd a nivel usuario     |
| Proxy inverso / TLS   | Caddy                                                 |
| Red                   | Pi-hole, Unbound                                      |
| Fotos / video         | Immich, PostgreSQL, Redis                             |
| Gestor de contraseñas | Vaultwarden                                           |
| CalDAV/CardDAV        | Radicale                                              |
| Archivos              | Syncthing, Filebrowser, Samba                         |
| Lecturas varias       | Miniflux + PostgreSQL, Suwayomi, Kavita, FlareSolverr |
...
Al implmentar un celular, hay que saber que no se maneja con ningún proceso init, pero si posee un sustituto como add on.

Instalá Termux y el add on Termux:Boot desde sus repositorios o desde F-Droid (nunca de la play store), y después, dentro de Termux:
...
Por último, de vuelta en Termux, copiá `s9+/.termux/boot/start-all.sh` a `~/.termux/boot/` y dale permiso de ejecución. Ahora al iniciar y abrir la aplicación de Termux:Boot, arranca `sshd`, `ntfy-start.sh` y `check-pi.sh` al bootear.
...
Cada pod se mapea como contenedores separados, en donde se orquestan Pods para compartir infraestructura entre ellos y así organizarlos a nivel de ficheros como de capacidades. Reiniciar o actualizar uno no afecta a los demás.

## Backups

Unicamente el servidor de datos posee backups periódicos ya que es el que posee información más relevante y en mayor cantidad. Se ejecuta thinkcentre/scripts/backup.sh por cron, recibiendo el directorio destino como único argumento:
...
La versión web linkeada arriba no forma parte de este repositorio. Se encuentra como una extensión de mi [portfolio](https://github.com/ncorrea-13/portfolio/tree/main/servidor) y sale a internet por un Cloudflare Tunnel.
